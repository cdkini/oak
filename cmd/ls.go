...
func list(root string) error {
	...
	color.Blue("%-*s %-*s %-*s %-*s\n", padding[0], "Title", padding[1], "Updated At", padding[2], "Created At", padding[3], "Tags")
	for _, m := range metadata {
		fmt.Printf("%-*s %-*s %-*s %-*s\n", padding[0], m.Title, padding[1], *m.UpdatedAt, padding[2], m.CreatedAt, padding[3], m.Tags)
	}
